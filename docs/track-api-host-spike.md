...
- formal run前のcalibrationでは、`open`、`press_key`、`set_value`、semantic click、single / double coordinate clickのexact contextで15 deltaが0となるnegative control、physical move-only acceptance、physical click / key / scroll / drag rejection、wrong-target clean-result rejectionに加え、通常key / mouse buttonをarm sampleまで保持して`KEY_HELD` / `MOUSE_BUTTON_HELD`となるheld-state rejectionと、arm sample中の連続pointer moveが`INPUT_DURING_SAMPLE`となるsample-race rejectionの**両方**を独立したfresh guard processで完了する。両sampling rejectionではoperator traceの物理入力区間が失敗sample全体を含む`call started <= sample started <= sample completed <= call ended`を機械検証する。closed calibration directoryは9 distinct fresh guard processについてguard JSONL / 空stderr / operator trace JSONLのexactly 27 top-level process file、exactly 30 state capture、exactly 30 screenshot captureだけを含む。
...
- [ ] exact-context calibrationにmove-only acceptance、wrong-target clean-result後のreject、physical click / key / scroll / drag rejectionと、独立したheld-state rejectionおよび独立した`INPUT_DURING_SAMPLE` pointer-move sample-race rejectionの両方があり、9 fresh guard process、exactly 27 top-level process file、exactly 30 state capture、exactly 30 screenshot captureを保存した
