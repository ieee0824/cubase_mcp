...
macOSのprimary runでは、Cubaseを停止したままcollectorを起動して`collector_started`を確認し、その後、最初のcheckpointを開始する前に`cubase_input_guard`を起動して`version = 2`、`source = hid_system_state`、`coverage = action_windows`、`privacy = counts_and_held_state_boolean`の`ready`を確認します。以後のguard response / errorも同じversionとcoverageを必須とし、欠落または不一致を拒否します。collector起動に必要なOS承認はguard startupより前に完了させ、guard startup後はcollectorを再起動しません。このsidecarはWindow Serverが保持する物理HID eventの累積countと、通常keyまたはmouse buttonが押下中かというaggregate booleanだけを読みます。どのkey code / buttonだったかを保持・serialize・logせず、入力文字、pointer座標、対象applicationも取得しません。各UI操作では、freshなpre-stateを取得する**前**に`arm`してその時点のcountをbaselineとし、単一のUI操作とfreshなpost-state確認を終えた直後に同じ`action_id`で`check`します。armからcheckまでの物理入力が1件でもあればそのactionとrunをfail closedにし、同じcheckpoint内でやり直しません。arm前のidle区間は次のbaselineへ繰り越しません。その区間にはautomationのUI actionを置かず、次のarm後にactive application、window、project、dialog、targetをfreshに再取得し、Probeのorphan callbackや予期しない状態変化があれば別途run invalidにします。これにより、待機中にpointerが移動しても古い座標を使わず、pre-state取得からclickとpost-state確認までに移動した場合だけ確実に拒否します。guard processはcollectorの正常summaryまで同一processで維持し、その後にだけ`finish`を送り、`coverage = action_windows`、`interference_detected = false`の`finished`を確認します。`interference_detected = true`、`cancelled`、guard protocol error、process終了、action ID不一致、明示的`finish`なしのEOF、またはarmedのままの`finish`は、その作用先をUIで推測できる場合でもrun invalidです。guardを途中で再起動してaction windowを継ぎ足しません。

`finished`が証明するのは、armed action windowがすべて閉じられ、そのwindow内のinterferenceがlatchされず、cancelもなかったことだけです。arm前のidle区間に物理入力がなかったことや、run全体への排他的入力は証明しません。`ping`と`finish`はidle差分を判定せず、held key / sample race / counter timeoutのfail-closed sampleは`arm`と`check`に適用します。

run開始前に、実際に注入するUI callを順序付き・重複なしの`ui-action-inventory`としてrepository外へ固定します。menuを開く、menu itemを選ぶ、dialog値を設定する、確定する、windowをraiseする、scrollする、clickする、keyを送る等は、それぞれ別のaction IDです。1つのarm/checkへ複数callを入れず、pre-stateまたはpost-state確認に失敗した場合は`cancel`してrunを無効にし、retry用の別actionを追加しません。run後はguard JSONLの`armed`と`result`のaction ID列がinventoryとexactly一致し、各IDが隣接する1 pairで、欠落・追加・逆順がないことを機械照合します。sidecar自体はComputer Useや別automation APIの直接callをinterceptしないため、この照合とoperator tool traceなしに「全UI callがguardされた」と主張しません。audit manifest v1 / report v2もこのinventoryをbindしないため、guard artifactはlocal operator evidenceに限ります。
