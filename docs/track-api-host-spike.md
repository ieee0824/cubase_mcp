...
- [ ] input guardの全recordがv4 `consequential_input_only` contract、session identity、連続record sequenceと一致し、inventoryとarmed / resultのID列が一致し、全resultの15 consequential deltaが0である。move-only acceptance、wrong-target clean-result後のreject、physical click / key / scroll / drag rejectionを含むexact-context calibrationも保存した
