...
同梱のCubase MIDI Remote adapterは、Bridge ProtocolのUTF-8 JSONを7-bit safeなSysExへ格納します。フレームは次の構成です。
...
- `payload`: JSONの各byteを上位4 bit、下位4 bitの順に2 byteへ分解したもの
- JSONサイズ上限: 64 KiB

この変換により、UTF-8 JSON内の8 bit値はSysEx data byteに直接出現しません。Request/Responseの`id`照合、Eventの割り込み、タイムアウトはTCP transportと同じです。デフォルトの仮想ポートは次の通りです。
