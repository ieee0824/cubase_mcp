...
Input / Outputには独立した[読み取り専用I/O Probeとcollector profile](io-existing-channels-probe.md)を用意しています。実機挙動は未確認で、実機観測は[Issue #33](https://github.com/ieee0824/cubase_mcp/issues/33)で扱います。primaryの結果へ混ぜず、構造検証と実機での観測・UI照合を区別します。
...
Cubase edition、runtimeが実際にloadしたAPI、run日時、About画面の完全な表示、repository commit、probe source / installer embedded / deployedとinput guard / collector / auditor binaryのSHA-256は各runで`PENDING`から置換します。13.0.50は未installなので、Cubase 15のrunを「13.0.50確認済み」として扱いません。

Probeの`host_version`は`mDefaults.mAppVersion.getVersionString()`由来のruntime profile補助値です。API v1.1のCubase 13型定義にはこのsurfaceがないため、C13 profileは`null`を正規の取得不能値として許可し、値が存在する場合は`13.0.30`またはprofile固定の完全buildだけを受理します。C15 profileは`null`を拒否し、`15.0.30`またはprofile固定の完全buildだけを受理します。正確なCubase buildはmanifestのAbout / app bundle証拠で別に固定し、Probe値または`null`から補完・推測しません。別patch、別build、prefix / suffix付き文字列を受理しません。
...
6. repository commit、probe source、installer embedded source、deployed probeのdigestを照合し、3つが一致しなければrunを開始しない。実行するinput guard、collector、auditorのrelease binary SHA-256も別々に固定する。
...
| input guard binary SHA-256 | frozen action inventoryへ固定するrelease binaryから取得 | `PENDING_RUNTIME` |
| collector binary SHA-256 | run開始時に実行するrelease binaryから取得 | `PENDING_RUNTIME` |
| auditor binary SHA-256 | final検証で照合するrelease binaryから取得 | `PENDING_RUNTIME` |
...
formal evidenceの最終入口はrepository側の`scripts/check-track-probe-evidence.sh`です。checkerはworktreeがcleanでHEADがfrozen `ui-action-inventory`のrepository commitと一致することを先に検証し、そのclean inventory-bound commitからinput guard、collector、auditorの3 binaryを隔離target directoryへ`--release --locked --offline`でrebuildします。再buildしたguardのSHA-256はinventory固定値、collector / auditorのSHA-256はrun record固定値と一致し、3つすべてが対応する通常のrelease binaryともSHA-256・byte単位の両方で一致しなければなりません。report生成にはその一致を確認した再build auditorだけを使います。evidence内のchecker copyはrepository側scriptとのbyte一致とdigest固定の対象ですが、実行入口にはしません。
...
両profileの必須checkpoint IDはfixtureで定義したexactly 44個です。各checkpointはbegin後・実操作sequence直前のraw `collector_action` markerをexactly 1件必要とします。cutを使う21 checkpointではcollectorが成功cut responseと、同request ID / epochかつ`boundary_source = probe.observation.cut_response`のaction markerをatomic output pairとして隣接出力し、operatorは別のmarkerを送らず、その自動marker直後に`arm`から始まるguarded UI action sequenceを開始します。このmarkerを5000 ms windowの監査anchorとします。E0 / E1 / E8 / C1 / S9-empty / S9-mutation / S9-baselineでは、同じmarkerの後にOS openと、論理操作`Activate`へ対応するexact affirmative semantic button clickを別々にguardした2 actionとして順に行い、2つ目のmarkerやcutを追加しません。このformal環境の日本語UIではdialog textを`プロジェクトをアクティブにしますか？`、affirmative identityを`ボタン 有効化, ID: action-button-1`、negative identityを`ボタン いいえ, ID: action-button-2`へ固定し、buttonはexactly 2個でなければなりません。E8 / C1 bank navigationではReset / Next / Prev成功response、`INIT` / `R1` / `R2`では明示的action markerがanchorです。final snapshotは対応anchorから5000 ms以上後でなければなりません。navigation checkpointはmarkerから1000 ms以内にID suffixどおりのProbe commandを送り、同じconfigの明示的`probe.bank.snapshot`を必要とします。navigation commandは63件のguarded UI callには含めません。それ以外の`INIT` / `E0` / `E1` / `E8` / `C1`、S0〜S9、R1 / R2はfinal `MB_CORE_ALL`と`MB_CORE_VISIBLE` bank snapshotを両方必要とします。C15-COMBINED、およびINITのruntime capabilityがDirectAccessをsupported / activeと報告したC13 runでは、各checkpointで`probe.direct_access.snapshot`を先に完了させ、その後に必要なbank snapshotを完了させます。C13で継続可能なunsupported分岐は`supported: false, active: false`かつ固定のunavailable / incomplete reason、DirectAccess eventなしだけです。`supported: true, active: false`、activation error、またはその他の組合せはMB-only成功へ読み替えずrun invalidとします。`INIT`はcheckpoint beginとaction markerの後にarm sample / `armed`、Cubase未起動のfresh pre-stateを順に完了してからCubaseを起動し、fresh post-stateとcheck sample / `result`を完了します。初期snapshot完了後にreadyとなったことを検証してからdiscoverとcapabilitiesを完了します。INITのcold launchはR1 / R2用の30秒deadlineへ含めません。R1 / R2はaction markerから30000 ms以内に新しいreadyと再discoverを完了させ、ready / discoveryから10000 ms以内にfinal snapshotを完了させます。
...
- macOS primary runでは、Cubase停止中にcollectorを起動して`collector_started`を確認した後、最初のcheckpointより前に同一processの`cubase_input_guard`を起動し、protocol `version = 5`、`source = hid_system_state`、`coverage = action_windows`、`privacy = counts_and_held_state_boolean`、`policy = consequential_input_only`を確認する。全recordで同じ`guard_session_id` / `guard_process_id` / `guard_started_at_unix_ms`、正の整数`recorded_at_unix_ms`、1始まりの連続`record_sequence`を要求し、`recorded_at_unix_ms`は非減少でtrace headerと結合する。`armed` / `result` recordと、`arm` / `check`のsampling中に生じたerror recordは、正の整数`sample_started_at_unix_ms` / `sample_completed_at_unix_ms`の両方を持ち、`sample_started_at_unix_ms <= sample_completed_at_unix_ms <= recorded_at_unix_ms`でなければならない。これらは相関IDと時刻であり、actor認証や署名ではない。collector起動のOS承認はguard startup前に完了させ、startup後はcollectorを再起動しない。各操作はactive checkpointのsingle markerより後、fresh pre-stateより前に`{"command":"arm","action_id":"..."}`を送り、`armed`を確認してから進める。UI callを行った場合はfresh post-state確認直後に必ず同じIDで`check`し、`result`を確認してからcheckpointを閉じる。postcondition不一致かつclean resultなら同じIDの`reject`を続け、`postcondition_failed`としてsessionをabortする。正常に完了したarm / check sample間の`mouse_moved`はinformationalであり、単独ではinterferenceにしない。left / right / other mouse down / up / drag、key down / up、flags changed、scroll wheel、tablet pointer / proximityの15 deltaはいずれも1件以上でfail closedとする。`HID_SYSTEM_STATE` counterはeventのactorや物理 / synthetic由来を認証しない。各sampleではper-type counterとheld-stateの読取り全体を`kCGAnyInputEventType` aggregate counterで前後から括り、その短い区間のaggregate変化はmove-onlyでも`INPUT_DURING_SAMPLE`として拒否する。held key / mouse buttonと`2000 ms` timeoutもfail closedにする。arm前のidle入力は次のbaselineへ繰り越さず、guard error / 終了 / ID不一致 / finish失敗 / cancelled / rejected / latchではrunを停止する。
- run前にexactly 63 UI callのordered action inventoryをrepository外へ固定し、menu open / item選択 / dialog入力 / 確定 / window raise / scroll / click / keyをそれぞれ別IDでarm/checkする。projectがactiveな状態から別projectへ切り替える7 checkpointでは、OS openと後続のexact affirmative semantic button clickも別IDにする。run前に決まるAPI、操作、click count、precondition、postconditionをinventoryへ固定し、dynamicなsemantic identityまたは座標はfresh pre-stateから解決して同じIDのoperator tool traceへ記録する。guard JSONLのarmed / result ID列とinventory、全recordのv5 identity / sequence / policy / record時刻、全sampling recordの開始・完了時刻、全resultの15 consequential deltaが0であることを機械照合し、欠落・追加・逆順を拒否する。formal evidenceはpre / post captureに対応するexactly 126個のstate JSON fileとexactly 126個のscreenshot file、およびsession header 1件 + action 63件 + session end 1件のexactly 65 JSONL trace recordだけを受理する。sidecarはautomation APIの直接callをinterceptしないため、inventory、exactly 1 callのtool trace、fresh pre / post-stateなしに全UI callの正しさを主張しない。
- formal checkerは各UI actionについて、`checkpoint begin -> single collector_action marker -> arm sample / armed record -> fresh pre -> exactly 1 call -> fresh post -> check sample / result record -> checkpoint end`の包含順序を要求する。operator traceのRFC 3339 millisecondをUnix millisecondへ変換し、`checkpoint begin <= marker <= armed sample start <= armed sample complete <= armed recorded <= pre < call start <= call end < post <= result sample start <= result sample complete <= result recorded <= checkpoint end`を照合する。外側の境界は同一millisecondを許すが、UI内部の`pre < call start <= call end < post`は厳密に保つ。複数actionのcheckpointでもmarkerはexactly 1件だけで、armからresultまでをaction IDごとに繰り返す。このcross-artifact対応付けはcollector、guard、operator traceが共有するsystem wall clockをtrusted boundaryとし、cryptographic clockでもtamper-proofな時刻証明でもない。
- formal run前のcalibrationでは、`open`、`press_key`、`set_value`、semantic click、single / double coordinate clickのexact contextで15 deltaが0となるnegative control、physical move-only acceptance、physical click / key / scroll / drag rejection、wrong-target clean-result rejectionに加え、通常key / mouse buttonをarm sampleまで保持して`KEY_HELD` / `MOUSE_BUTTON_HELD`となるheld-state rejectionを独立したfresh guard processで完了する。別にOS readをtest buildで置換した決定的sampling contractテストを実行し、実機raceの再現とは区別する。held-state rejectionではoperator traceの物理入力区間が失敗sample全体を含む`call started <= sample started <= sample completed <= call ended`を機械検証する。closed calibration directoryは8 distinct fresh guard processについてguard JSONL / 空stderr / operator trace JSONLのexactly 24 top-level process file、exactly 28 state capture、exactly 28 screenshot captureだけを含む。
- `begin`後、実操作sequence直前に同じIDのaction markerをexactly 1回記録する。`E0` / `E1` / `E8` / `C1`とS0〜S9のexactly 21 checkpointでは、最初のtargeted commandとして`probe.observation.cut`を`@selected`へ送る。成功responseはcollectorがraw responseと同request ID / epochのaction markerをatomic output pairとして隣接出力するため、operatorは`collector.action`を重ねず、自動marker直後に`arm`からguarded UI action sequenceを開始する。7つのproject切替checkpointはOS openのguardを閉じてからactivate-project用guardをarmし、fresh dialogからexact affirmative semantic buttonを1回clickする。`INIT` / `R1` / `R2`と、Reset / Next / Prevが新しいbank generationを作るE8 / C1 navigationではcutを送らず、明示的`collector.action`を使う。navigation Probe commandはmarkerから1000 ms以内に送るが、UI callではないためarm / pre / post / check evidenceを付けず、63件のinventoryへ含めない。cut checkpointと`INIT` / `R1` / `R2`はaction marker、navigationは操作成功responseを観測anchorとし、対応anchorから5000 ms以上を経過させた後、checkpoint内で利用中の各projectionへ明示的final snapshot commandを送る。DirectAccessがactiveならDirectAccessを最初に完了し、同期`update()`が生むfeedbackもその完了前に収め、その後に必要なbank projectionを送る。最初のfinal snapshot完了後は、後続する同じsetの明示的snapshot request / response / chunk以外のprobe messageやhost callbackを許さない。set最後のresponse / 全chunk完了後から`end`までもprobe messageもhost callbackも1件もない状態で1000 ms以上quietを満たし、UI annotationを完了してからだけ`end`を送る。set途中または最後のsnapshot後に予期しないmessageが届いたrunは、quiet timerだけを再開して成功扱いにしない。
...
E0 / E1 / E8 / C1やS9のproject切替を含む通常checkpointでも、hostが同じscript sourceをpage deactivate / reactivateする場合があります。その場合は固定済みのopen + activation click以外にlifecycle用UI操作を追加せず、同じcheckpointのopen / activation click sequenceに伴う`probe.ready(false)` → 同sourceの`probe.mapping_active` → capability → 全初期snapshot → `probe.ready(true)`をexactly 1 sequenceとして完了させ、再discoverしてからfinal snapshotへ進みます。旧activationで既に受信したbounded feedbackは`probe.ready(false)`より前に全件送信し、64件の1 idle batchを超えても最大queue件数まで有限回でdrainします。初期化中のDirectAccess change callbackもfeedbackとして保存しますが、`page_activate` snapshotが同じ初期状態を覆うため、ready前に別の自動change snapshotをscheduleしません。page境界で無効になるcoalescedな非command DirectAccess change snapshotだけはcancelし、明示command、`page_activate`、bank snapshot、未知reason、または未drain feedbackを残したままinactiveへ移行しません。新しい`probe.loaded`はINIT / R1 / R2以外では許可しません。lifecycleが無いrunへこのsequenceを捏造せず、partial、複数回、action前sequenceの流用、再discoverなしをauditorが拒否します。
...
2. offline validationをすべて実行し、repository commit、probe source SHA-256、実行するinput guard / collector / auditor release binary SHA-256をlocal evidenceへ別々に固定する。guard digestはfrozen action inventory、collector / auditor digestはrun recordへ保持する。fixtureで定義した、対象versionで新規作成済みのINIT bootstrapについても、0 Project Track、正式E0とは別file、run前SHA-256を確認する。対象versionを起動するexactなapplication名またはbundle pathとbootstrapのabsolute pathをlocalで確定し、absolute pathをraw JSONL、manifest、run ID、共有report、repositoryへ転記しない。
...
4. repository外を出力先にし、Cubaseを停止したまま`--run-id <RUN_ID>`付きでcollectorを先に起動する。最初の`collector_started` recordとCubase未起動を確認した後、macOS primary runでは入力guardを起動して`version = 5`、`source = hid_system_state`、`coverage = action_windows`、`privacy = counts_and_held_state_boolean`、`policy = consequential_input_only`の`ready`を確認し、以後のresponse / errorでもversion、coverage、policy、session identity、record sequence、Unix-ms fieldの一致を要求する。collector起動に必要なOS承認はguard startup前に完了させ、startup後はcollectorを再起動しない。次にexact ID `INIT`の初期化checkpointを開始し、single `collector.action` markerを記録する。そのmarkerより後に`INIT.launch-bootstrap`をarmし、arm sampleと`armed` recordの完了を確認してからCubase未起動のfresh pre-stateを取得する。revision 2 primary C13 / C15 runではfixtureで定義した`CMCP_TrackFixture_Bootstrap_Empty.cpr`を例外なく使う。fresh pre-stateの後、step 2で確定したexactな製品とbootstrap documentをexactly 1回の同じOS launch操作へ指定する。macOSでは`open -a "<EXACT_CUBASE_APPLICATION_OR_BUNDLE_PATH>" "<ABSOLUTE_BOOTSTRAP_PROJECT_PATH>"`のように両pathをquoteする。launch後のfresh post-stateを取得してから同じaction IDのcheck sample / `result`を完了し、その後にだけcheckpointを終了できる。bootstrapは0 Project Trackかつmedia、event、part、automation、plug-in、user preset、新しいrouting設定、通常project由来の設定を持たず、実機inputをmonitorしないINIT専用projectであり、fixture caseや独立checkpointではない。bootstrap用annotationを増やさず、既存`INIT` annotationを使うため、manifest v1 schemaとexactly 44 checkpoint IDは変更しない。
5. 初期化checkpoint内で対象Cubaseを1 instanceだけ起動してprobeを新規loadする。CubaseだけをHubへ先に起動した後のbootstrap open、別projectを経由したopen、またはprimary INIT中の後付け救済は禁止する。bootstrapが同じlaunch操作で開かなかった場合はrunを停止し、graceful drain後にfresh run ID、fresh collector process、fresh JSONLでINITから再実施する。INIT終了まで別projectを開かず、Project windowのexact basename、0 Project Track、他projectなし、dirty / modified表示なしをUIで確認する。この単一launchを行った場合だけ既存INIT annotationの`action_confirmed`、すべてのUI条件とbootstrap未変更を確認した場合だけ`ui_ground_truth_confirmed`を`true`にする。process不在からのINITはexactly 1 open actionであり、Activate dialogや追加promptが現れた場合はrun invalidとする。bootstrapのevidenceやannotationをE0へ流用しない。R1ではcollectorを動かしたままreload用checkpointを開始し、fixtureの`R1.studio-menu`から`R1.reload-scripts`までの4 actionを順に実施する。R2も`R2.cubase-menu`、`R2.quit-item`、`R2.launch-baseline`の3 actionとする。各checkpointのmarkerは1件だけとし、その後の各actionを独立したarm / armed → fresh pre → exactly 1 call → fresh post → check / resultでguardする。
...
9. fixtureの再現性checklistを完了し、E0、E1、E8、C1、M1を順に観測する。exact ID `E0`を開始して`probe.observation.cut`の成功responseと隣接する自動action markerを確認し、その後に`E0.open-project`のarm sample / `armed`とfresh pre-stateを完了してから、bootstrapとは別fileの正式な`CMCP_TrackFixture_Empty.cpr`をexactly 1回のcallでOS openする。active projectがある状態で別projectを開くCubaseの固定挙動に従い、open actionのfresh post-stateとしてE0固有のactivation確認dialog、exactly 2 button、唯一のenabled affirmative semantic target `ボタン 有効化, ID: action-button-1`を確認し、check sample / `result`でそのguardを閉じてから別action IDをarmし、freshに再解決した同じtargetを1回clickする。Project windowでbasename `CMCP_TrackFixture_Empty`と0 Project Trackを目視確認できた場合だけE0の`action_confirmed` / `ui_ground_truth_confirmed`を`true`にする。E1 / E8 / C1とS9-empty / S9-mutation / S9-baselineもfixture表のsource / targetに対して同じ2 action sequenceを必須とする。dialog欠落、直接activation、target不一致、buttonの文言・role・ID・enabled状態の不一致、affirmative targetが0件または複数、negativeまたは別controlへの作用、追加promptは、dismiss / retry / skipせずfail closedにする。bootstrapのsnapshotはE0 final snapshotの代用にしない。case open / snapshotはexact ID `E0`、`E1`、`E8`、`C1`、E8 navigationは`E8-<CONFIG>-<STEP>`、C1 navigationは`C1-<CONFIG>-<STEP>`、mutation / project切替はS0〜S9を使い、44 checkpointへ新しいIDを追加しない。project切替等で同sourceのreactivationが発生した場合は同checkpointのopen / activation click sequenceに伴う`ready(false)` → mapping → capability → 全初期snapshot → readyをexactly 1回完了し、再discoverを同じcheckpoint内で行ってからfinal snapshotへ進む。通常checkpointの新しい`probe.loaded`、partial / 複数reactivation、action前sequenceの流用、再discover省略はrun invalidとする。O1はaudit v1 / primary Probeに含めず`skipped / not_separately_authorized`固定とする。
10. 各bank / mutation操作を固有checkpointで囲む。E0 / E1 / E8 / C1とS0〜S9はobservation cutの成功responseと隣接する自動action markerを確認し、別のmarkerを送らず、その直後に`arm`からguarded UI action sequenceを開始する。上記7 checkpointのopen / activation clickは各UI callを独立してguardするが、同じcheckpoint markerと5000 ms windowを共有する。E8 / C1 navigationは明示的marker後1000 ms以内に対応するProbe commandを送り、UI actionのguard evidenceや63件のinventoryへ混ぜない。cut checkpointは自動marker、navigationは操作成功responseを観測anchorとして5000 ms以上待った後に明示的final snapshotを要求する。Cubase 15またはC13でDirectAccessがsupported / activeならDirectAccessを最初に取得し、navigationはその後に操作config、その他は`MB_CORE_ALL`と`MB_CORE_VISIBLE`の両方を取得する。最後のresponse / chunk後に新しいmessageがないこと、追加1000 ms quiet、UI annotation、sequence integrityを確認してから次の操作へ進む。
11. R1 reloadとR2 restartは独立phaseとして実施する。R1のpre-stateは`S9-baseline`、R2のpre-stateはR1の監査済みfinal snapshotを参照し、R1/R2内に追加のpre-action snapshot commandを送らない。R2では正常終了後のprocess不在をfreshに確認してから、exactなCubase applicationとuntouched C1を`R2.launch-baseline`のexactly 1 OS open actionへ同時指定する。既存projectがactiveではないためActivate actionを追加せず、Activate dialogまたは他のpromptが現れた場合はrunを停止する。
...
14. repository側のfinal checkerをcleanなinventory-bound commitで実行し、input guard / collector / auditorの隔離rebuildとbyte / SHA-256一致を確認した後、そのcheckerが再build auditorへraw JSONLとaudit manifest v1を同時入力して必須coverageとredaction検査を通過させる。この文書にはredacted audit report v2（`audit_report_version = 2`）の集計と最小限の合成名例だけを転記し、raw JSONLを移動・copy・stageしない。
...
fixtureに従い、cut対象操作sequenceはcut成功responseとatomic pairで隣接する自動action marker直後、navigationとlifecycle操作は明示的marker直後に開始し、checkpoint種別ごとの観測anchorから必ず`5000 ms`観測します。7つのproject切替ではOS openとActivateの両actionを同じwindowへ含めます。checkpoint begin、cut response、navigation commandより前のmarkerからの経過時間で代用せず、最初のquiet periodで早期終了しません。
...
E8はprimary bank幅とeligibleなProject由来channel数がexactly 8の独立fixtureです。最初の`E8` checkpointでprojectをOS openし、activation確認dialogを検証してexact affirmative semantic buttonを別actionでclickした後、両bank configのfinal snapshotを取得します。続くnavigation各行では操作成功responseから5000 ms以上を観測した後、操作configの明示的bank final snapshotを取得します。C15-COMBINED、およびruntimeでDirectAccessがactiveだったC13 runではDirectAccess final snapshotを先に取得し、その後に必要なbank final snapshotを取得します。最後のsnapshot完了後に追加1000 msのmessage-free quiet periodを置き、同じUI ground truthに対する別projectionとして記録します。

| checkpoint ID | expected UI ground truth | C13-MB | C13-DA conditional | C15-MB projection | C15-DA projection |
| --- | --- | --- | --- | --- | --- |
| `E8` | E8 projectをOS openし、exact affirmative semantic buttonを別actionでclickした後、E8-01〜E8-08の8 Audioだけが存在 | `PENDING` | `PENDING_CONDITIONAL` | `PENDING` | `PENDING` |
...
| S9-empty | E0をOS open → exact affirmative semantic buttonを別actionでclick | `PENDING` | `PENDING_CONDITIONAL` | `PENDING` | `PENDING` |
| S9-mutation | M1をOS open → exact affirmative semantic buttonを別actionでclick | `PENDING` | `PENDING_CONDITIONAL` | `PENDING` | `PENDING` |
| S9-baseline | C1をOS open → exact affirmative semantic buttonを別actionでclick | `PENDING` | `PENDING_CONDITIONAL` | `PENDING` | `PENDING` |
...
| R2 restart | 正常終了、process不在確認、C1を伴うexactly 1 OS open、Activate dialogなし、新process / session境界 | `PENDING` | `PENDING_CONDITIONAL` | `PENDING` | `PENDING` |
...
- [ ] repository commit、input guard / collector / auditor binary digestを別々に固定し、probe source / installer embedded / deployedの3 digestが一致している
...
- [ ] 各profileのexactly 44 checkpointについてbegin後のsingle action markerを確認し、cut対象では成功response直後のatomic pairに同request / epochの自動markerがあり、manual markerを重ねていない。全UI actionが`checkpoint begin <= marker <= arm sample / armed <= fresh pre < exactly 1 call <= fresh post <= check sample / result <= checkpoint end`に含まれ、UI内部は`pre < call start <= call end < post`であり、種別ごとの観測anchorから5000 ms以上の観測、明示的final snapshot、その後にmessageがない追加1000 ms quiet period、UI annotation、sequence gap判定を適用した
- [ ] ordered inventoryがexactly 63 action、operator traceがexactly 65 record、pre / post captureに対応するstate JSON fileとscreenshot fileがそれぞれexactly 126個であり、E0 / E1 / E8 / C1 / S9-empty / S9-mutation / S9-baselineのOS openとexact affirmative semantic buttonを別々にguardした。INITとprocess不在後のR2 launchは各1 open actionで、activation dialogや追加promptを許可していない
- [ ] input guardの全recordがprotocol v5 `consequential_input_only` contract、session identity、連続record sequenceと一致し、各recordに`recorded_at_unix_ms`、全`armed` / `result`と`arm` / `check` sampling errorにsample開始・完了Unix-msがある。inventoryとarmed / resultのID列が一致し、全resultの15 consequential deltaが0である
- [ ] exact-context calibrationにmove-only acceptance、wrong-target clean-result後のreject、physical click / key / scroll / drag rejectionと、独立したheld-state rejectionがあり、別に決定的sampling contractテストを実行し、8 fresh guard process、exactly 24 top-level process file、exactly 28 state capture、exactly 28 screenshot captureを保存した
- [ ] repository側のfinal checkerをcleanなinventory-bound commitから実行し、input guard、collector、auditorの3 binaryを隔離offline rebuildした。guard rebuildのSHA-256がinventory固定値、collector / auditor rebuildのSHA-256がrun record固定値と一致し、3つすべてが対応するrelease binaryともSHA-256・byte単位の両方で一致することを確認した
