...
- UI操作は対象Cubase instanceへの排他的入力として扱い、可能な場合はmouse pointer位置に依存しないsemantic targetを使う。操作直前にactive app / window / project / dialog / targetを再取得し、直後に期待したUI状態とProbe差分を再確認する。座標しか使えない場合は現在pointerの移動とclickを分離せず、直前のwindow screenshotへ束縛された絶対座標clickを単一操作として送る。clickを取得済みwindowへ束縛できない実装や別operatorのpointer移動で作用先が変わり得る実装は座標操作へ使わない。macOS primary runでは、Cubase停止中にcollectorを起動して`collector_started`を確認した後、最初のcheckpointより前に`cubase_input_guard`を起動し、同一processをcollector summary後まで維持する。collector起動のOS承認はguard startup前に完了させ、startup後はcollectorを再起動しない。各操作のfresh pre-stateより前に`arm`、post-state確認直後に同じaction IDで`check`し、このaction window内の物理HID countを検査する。arm前のidle入力は次のbaselineへ繰り越さない代わりに、その区間ではautomation UI actionを行わず、arm後のfresh UI stateとProbeのorphan / callbackを必ず検査する。action window内で物理HID countが1件でも増えた場合、guard error / 終了 / ID不一致 / finish失敗の場合、または別operatorの入力、focus / window / targetの移動、対象外への作用、誤clickの可能性があればannotationを成功へせずrunを停止し、同じcheckpoint内で再clickしない。input guardはsoftware由来のfocus変化を検出しないため、前後のUI / Probe確認を省略しない。pointer移動自体を取得できない実装も、意図した座標だけから成功を推測しない。
- run前に実際のUI callごとのordered action inventoryをrepository外へ固定し、menu open / item選択 / dialog入力 / 確定 / window raise / scroll / click / keyをそれぞれ別IDでarm/checkする。guard JSONLのarmed/result ID列とinventoryをrun後に機械照合し、欠落・追加・逆順を拒否する。sidecarはautomation APIの直接call自体をinterceptしないため、inventory照合とoperator tool traceなしに全UI callのguard適用を主張しない。
